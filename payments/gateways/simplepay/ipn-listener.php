...
namespace MPHB\Payments\Gateways\Simplepay;
...
class IpnListener extends Gateways\AbstractNotificationListener {
	...
	protected function process(){
		...
		if ( has_action( "mphb_simplepay_{$txnType}" ) ) {
			// Allow Simplepay IPN types to be processed separately
			do_action( "mphb_simplepay_{$txnType}", $this->input, $this->payment->getId() );
		} else {
...
//				break;
	...
	private function checkCurrencyCode(){
		...
		if ( $currencyCode !== strtolower( $this->payment->getCurrency() ) ) {
			$log = __( 'Payment failed due to invalid currency in Simplepay IPN.', 'motopress-hotel-booking' );
	...
	private function checkPaymentAmount(){
		...
		if ( number_format( (float) $this->input['mc_gross'], 2 ) < number_format( (float) $this->payment->getAmount(), 2 ) ) {
			$log = __( 'Payment failed due to invalid amount in Simplepay IPN.', 'motopress-hotel-booking' );
			$this->paymentFailed( $log );
			return false;
		}
	...
	private function checkPaymentKey(){
		...
		if ( $this->input['invoice'] !== $this->payment->getKey() ) {
			$log = __( 'Payment failed due to invalid purchase key in Simplepay IPN.', 'motopress-hotel-booking' );
			$this->paymentFailed( $log );
			return false;
		}
	...
	private function retrievePendingReasonNote(){
		...
		switch ( strtolower( $this->input['pending_reason'] ) ) {
			...
			case 'address' :
				...
				$note = __( 'Payment requires a confirmed customer address and must be accepted manually through Simplepay.', 'motopress-hotel-booking' );

				break;

			case 'intl' :

				$note = __( 'Payment must be accepted manually through Simplepay due to international account regulations.', 'motopress-hotel-booking' );

				break;

			case 'multi-currency' :

				$note = __( 'Payment received in non-shop currency and must be accepted manually through Simplepay.', 'motopress-hotel-booking' );
			...
			case 'regulatory_review' :
				...
				$note = __( 'Payment is being reviewed by Simplepay staff as high-risk or in possible violation of government regulations.', 'motopress-hotel-booking' );
			...
			case 'upgrade' :
				...
				$note = __( 'Simplepay account must be upgraded before this payment can be accepted.', 'motopress-hotel-booking' );

				break;

			case 'verify' :

				$note = __( 'Simplepay account is not verified. Verify account in order to accept this payment.', 'motopress-hotel-booking' );

				break;

			case 'other' :

				$note = __( 'Payment is pending for unknown reasons. Contact Simplepay support for assistance.', 'motopress-hotel-booking' );
	...
	public function paymentRefunded( $refundLog = '' ){
		...
		if ( number_format( $refundAmount, 2 ) < number_format( (float) $this->payment->getAmount(), 2 ) ) {
			$log = sprintf( __( 'Partial Simplepay refund processed: %s', 'motopress-hotel-booking' ), empty( $this->input['parent_txn_id'] ) ? $this->input['parent_txn_id'] : ''  );
		...
		if ( !empty( $this->input['parent_txn_id'] ) && !empty( $this->input['reason_code'] ) ) {
			$reasonLog = sprintf( __( 'Simplepay Payment #%s Refunded for reason: %s', 'motopress-hotel-booking' ), $this->input['parent_txn_id'], $this->input['reason_code'] );
			$this->payment->addLog( $reasonLog );
		}

		$transactionLog = sprintf( __( 'Simplepay Refund Transaction ID: %s', 'motopress-hotel-booking' ), $this->input['txn_id'] );
